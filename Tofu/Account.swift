...
final class Account {
  ...
  convenience init?(url: URL) {
    let label = url.path.trimmingCharacters(
        in: CharacterSet(charactersIn: "/"));
      guard let host = url.host, host == "hotp" || host == "totp"
      else { return nil }
    let labelComponents = label.characters.split { $0 == ":" }.map(String.init)
    guard labelComponents.count > 0,
      let components = URLComponents(url: url, resolvingAgainstBaseURL: false),
      let queryItems = components.queryItems, queryItems.count > 0 else { return nil }

    self.init()

    name = labelComponents.last?.trimmingCharacters(
    ...
    for queryItem in queryItems {
      ...
      case "secret":
        guard let secretString = queryItem.value,
          let secret = Data(base32EncodedString: secretString) else { break }
